...
(function () {
  ...
  //
  // 1) Inject CSS so the closed <select> & its <option>s match Claude’s dark input
  //
  const popupStyles = document.createElement('style');
  popupStyles.textContent = `
    /* closed dropdown matches the chat input */
    ...
    /* native options list (may still be white on some browsers) */
  ...
  //
  // 3) Inject the styled <select> + SVG button into Claude’s input field
  //
